...
def plot_losses(mre, title = '', save_name = None):
    ...
    # Labels & title
    ax.set_xlabel('Epoch')
    ax.xaxis.set_major_locator(MaxNLocator(integer=True)) # IntegWer x-axis ticks
    ax.set_ylabel('Loss')
    ax2.set_ylabel('LR')   
    ax2.set_yscale('log')
    ax.set_title(title)
    ...
    if save_name is not None:
        plt.savefig(f'../output/figs/loss/loss{save_name}.png',bbox_inches='tight')
...
#             else:
#                 im = ax.imshow(plot, cmap = cmap[i], norm = LogNorm(vmin = zlogmin, vmax = zlogmax), **imkwargs)
        else:
