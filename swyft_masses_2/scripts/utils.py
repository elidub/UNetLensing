...
def get_dataset_path(m, nsub, nsim, system_name, sigma):
    dataset_name = f'_M_m{m}_nsub{nsub}_nsim{nsim}_sigma{sigma}'
    dataset_path = f'/nfs/scratch/eliasd/dataset{dataset_name}.pt' 
    print(f'Dataset {dataset_name} exists!') if os.path.exists(dataset_path) else print('Dataset does not exist!')
    return dataset_name, dataset_path

def get_mre_path(sim_name, nmbins, sigma, lr, factor, patience, save_dir = '../data/mre'):
    save_id = f'nmbins{nmbins}_sigma{sigma}_lr{np.log10(lr)}_fac{np.log10(factor)}_pat{patience}'
