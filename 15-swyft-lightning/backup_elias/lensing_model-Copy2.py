...
from pyrofit.lensing.lenses import SPLELens, SPLEwithHaloes, KeOpsNFWSubhaloes, KeOpsNFWLens
...
def image_generator_sub2(x, y, phi, q, r_ein, slope, x_src, y_src, phi_src, q_src, index, r_e, I_e, zsub):
    res = 0.0125*8*50/NPIX_IMG       # resolution in arcsec_
    nx, ny = NPIX_IMG, NPIX_IMG                   # number of pixels
    X, Y = get_meshgrid(res, nx, ny)    # grid
...
def image_generator_sub(x, y, phi, q, r_ein, slope, x_src, y_src, phi_src, q_src, index, r_e, I_e, z_sub):
    res = 0.0125*8*50/NPIX_IMG       # resolution in arcsec
    nx, ny = NPIX_IMG, NPIX_IMG                   # number of pixels
    X, Y = get_meshgrid(res, nx, ny)    # grid
    ...
    x_sub, y_sub, M_sub = z_sub.T
    
    
#     z_lens, z_src = torch.tensor((0.5)), torch.tensor((2.))
#     c_200c = torch.tensor((c_200c), device = 'cuda')
#     nsub = torch.tensor((5), device = 'cuda')
...
#     print(c_200c.device)
    ...
    sub = KeOpsNFWSubhaloes(z_lens=z_lens, z_src=z_src,
                            nsub=nsub,pos_sampler=pos_sampler, mass_sampler=mass_sampler, c_200c_sampler=c_200c,
                            device='cuda')
